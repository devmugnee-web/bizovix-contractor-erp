...
export function DashboardKpiCard({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[clamp(88px,15.5vh,112px)] min-w-0 flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-gradient-to-br from-white via-white to-slate-50/50 p-2 shadow-[0_6px_18px_rgba(15,23,42,0.055)] transition-all duration-200 hover:-translate-y-px hover:shadow-[0_10px_24px_rgba(15,23,42,0.085)] 2xl:h-[132px] 2xl:p-3",
        accentClassName,
      )}
    >
      ...
      <div className="flex min-h-6 min-w-0 items-center gap-1.5">
        ...
        <span
          title={title}
          className="min-w-0 flex-1 whitespace-nowrap text-[9px] font-bold leading-none tracking-[-0.025em] text-slate-700 sm:text-[9.5px] lg:text-[10px] xl:text-[10.5px] 2xl:text-[12px]"
        >
          {title}
        </span>
      </div>
      ...
      {children && (
        <div
          className="mt-auto flex min-h-[18px] items-end justify-center whitespace-normal text-center text-[8px] font-semibold leading-[1.2] text-slate-600 sm:text-[8.25px] lg:text-[8.5px] xl:text-[9px] 2xl:min-h-[24px] 2xl:text-[10.5px]"
        >
          {children}
        </div>
      )}
