...
function mutation(path:string,method:"POST"|"PATCH"="POST"){return ()=>{const qc=useQueryClient();return useMutation<unknown,Error,Record<string,unknown>>({mutationFn:(body)=>apiRequest(path,{method,body:JSON.stringify(body)}),onSuccess:()=>qc.invalidateQueries({queryKey:key})});};}
...
export function useUpdateChequeStatus(id:string){const qc=useQueryClient();return useMutation<unknown,Error,Record<string,unknown>>({mutationFn:(body)=>apiRequest(`/cash-bank/cheques/${id}/status`,{method:"PATCH",body:JSON.stringify(body)}),onSuccess:()=>qc.invalidateQueries({queryKey:key})});}
