...
function mutation(path:string,method:"POST"|"PATCH"="POST"){return ()=>{const qc=useQueryClient();return useMutation<unknown,Error,Record<string,unknown>>({mutationFn:(body)=>apiRequest(path,{method,body}),onSuccess:()=>Promise.all([qc.invalidateQueries({queryKey:key}),...(path==="/cash-bank/accounts"?[qc.invalidateQueries({queryKey:["reminders"]})]:[])])});};}
export const useCreateBankAccount=mutation("/cash-bank/accounts");
export function useUpdateBankAccount(){const qc=useQueryClient();return useMutation<unknown,Error,{id:string;body:Record<string,unknown>}>({mutationFn:({id,body})=>apiRequest(`/cash-bank/accounts/${id}`,{method:"PATCH",body}),onSuccess:()=>Promise.all([qc.invalidateQueries({queryKey:key}),qc.invalidateQueries({queryKey:["reminders"]})])});}
export function useDeleteBankAccount(){const qc=useQueryClient();return useMutation<unknown,Error,string>({mutationFn:(id)=>apiRequest(`/cash-bank/accounts/${id}`,{method:"DELETE"}),onSuccess:()=>Promise.all([qc.invalidateQueries({queryKey:key}),qc.invalidateQueries({queryKey:["reminders"]})])});}
