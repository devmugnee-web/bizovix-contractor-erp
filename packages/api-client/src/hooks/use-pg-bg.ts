...
export function useEligiblePgBgTenders(query: PgBgEligibleQuery) {
  return useQuery({
    ...
    queryFn: () =>
      apiRequestPaginated<EligiblePgBgTender>("/pg-bg/eligible-tenders", { params: { ...query } }),
...
export function usePgBgWorkflowByDocument(documentPurchaseId: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiRequest<PgBgWorkflow | null>(`/pg-bg/workflows/by-document/${documentPurchaseId}`),
...
export function useWorkCategories() {
  return useQuery({
    queryKey: queryKeys.workCategories,
    queryFn: () => apiRequest<string[]>("/pg-bg/work-categories"),
  });
}
...
export function useOrganizationContacts(organizationMasterId?: string) {
  return useQuery({
    ...
    queryFn: () =>
      ...
      >("/pg-bg/organization-contacts", { params: { organizationMasterId } }),
...
export function useSavePgBgDraft() {
  ...
  return useMutation({
    mutationFn: (payload: SavePgBgWorkflowInput) =>
      apiRequest<PgBgWorkflow>("/pg-bg/workflows", { method: "POST", body: payload }),
...
export function useAcceptNoa() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiRequest<PgBgDecisionResult>(`/pg-bg/workflows/${id}/accept-noa`, {
        method: "POST",
        body: { acceptNoa, pgBgRequired },
      }),
    onSuccess: async () => {
      await Promise.all([
...
export function useFinalizePgBg() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: FinalizePgBgInput }) =>
      apiRequest<FinalizePgBgResult>(`/pg-bg/workflows/${id}/finalize`, {
        method: "POST",
        body: payload,
      }),
    onSuccess: async () => {
