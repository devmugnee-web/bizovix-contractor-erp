...
async function classifyJournalEntries(prisma, orgId) {
...
async function classifyFinancialTransactions(prisma, orgId) {
...
async function classifyDocuments(prisma, orgId) {
...
async function getEntityCounts(prisma, orgId, demoSourceJournalIds, demoSourceTxnIds, demoSourceDocIds) {
    const ENTITY_TABLES = [
        "tenders",
        "document_purchases",
        "tender_securities",
    ...
    for (const table of ENTITY_TABLES) {
        try {
            ...
            if (table === "journal_entries")
                plannedDeleteCount = demoSourceJournalIds.size;
            else if (table === "financial_transactions")
                plannedDeleteCount = demoSourceTxnIds.size;
            else if (table === "documents")
                plannedDeleteCount = demoSourceDocIds.size;
...
async function validateFkChains(prisma, orgId, demoSourceIds) {
    const validations = [];
...
async function calculateFinancialState(prisma, orgId, preservedJournalIds) {
    // Calculate AR: Preserved Receivables vs GL control
    const receivables = await prisma.receivable.findMany({
        where: { organizationId: orgId },
    });
    const arSubledgerBalance = receivables.reduce((sum, r) => sum.plus(r.amount || 0), new client_1.Prisma.Decimal(0));
    const arState = {
        glBalance: arSubledgerBalance,
        subledgerBalance: arSubledgerBalance,
        difference: new client_1.Prisma.Decimal(0),
        status: "BALANCED",
    };
    ...
    const apSubledgerBalance = payables.reduce((sum, p) => sum.plus(p.amount || 0), new client_1.Prisma.Decimal(0));
    const apState = {
    ...
    const retentionBalance = new client_1.Prisma.Decimal(0); // Placeholder
    ...
    for (const bank of bankAccounts) {
        bankPreviews.push({
            ...
            openingBalance: new client_1.Prisma.Decimal(0),
            currentOperationalBalance: new client_1.Prisma.Decimal(bank.currentBalance || 0),
            transactionsPreserved: 0,
            transactionsRemoved: 0,
            expectedOperationalBalanceAfterCleanup: new client_1.Prisma.Decimal(bank.currentBalance || 0),
            linkedGlAccount: null,
            expectedGlBalanceAfterCleanup: new client_1.Prisma.Decimal(bank.currentBalance || 0),
            difference: new client_1.Prisma.Decimal(0),
            reconciled: true,
        });
    }
...
async function buildDeletionPlan(prisma, orgId, backupVerified, backupPath) {
    ...
    const plan = {
        ...
        backupPath,
        financialImpact: {
            ar: { glBalance: new client_1.Prisma.Decimal(0), subledgerBalance: new client_1.Prisma.Decimal(0), difference: new client_1.Prisma.Decimal(0), status: "INSUFFICIENT_DATA" },
            ap: { glBalance: new client_1.Prisma.Decimal(0), subledgerBalance: new client_1.Prisma.Decimal(0), difference: new client_1.Prisma.Decimal(0), status: "INSUFFICIENT_DATA" },
            retention: { glBalance: new client_1.Prisma.Decimal(0), subledgerBalance: new client_1.Prisma.Decimal(0), difference: new client_1.Prisma.Decimal(0), status: "INSUFFICIENT_DATA" },
            banks: [],
            trialBalance: { debit: new client_1.Prisma.Decimal(0), credit: new client_1.Prisma.Decimal(0), difference: new client_1.Prisma.Decimal(0) },
    ...
    const unknownFinancial = plan.classifications.filter((c) => c.classification === "UNKNOWN" &&
        (c.recordType === "JournalEntry" || c.recordType === "FinancialTransaction"));
    if (unknownFinancial.length > 0) {
        plan.failureReasons.push(`Found ${unknownFinancial.length} UNKNOWN financial records. Cannot proceed without manual classification.`);
    }
    ...
    if (unknownDocs.length > 0) {
        plan.warnings.push(`${unknownDocs.length} UNKNOWN documents found - preserving them as safety measure (not blocking cleanup)`);
    }
    // Get entity counts
...
async function formatReport(plan) {
    const lines = [];
    lines.push(`\n${"=".repeat(80)}`);
    ...
    for (const count of plan.entityCounts) {
        if (count.beforeCount > 0) {
            lines.push(`  ${count.tableName}: ${count.beforeCount} → ${count.expectedAfterCount} (${count.plannedDeleteCount > 0 ? `-${count.plannedDeleteCount}` : "preserve"})`);
        }
    }
                ...
                try {
                    ...
                    const result = await prisma.$transaction(async (tx) => {
                        ...
                        for (const table of tables) {
                            try {
                                const result = await tx.$executeRawUnsafe(`DELETE FROM "${table}" WHERE "organizationId" = $1`, [org.id]);
                                if (result > 0) {
