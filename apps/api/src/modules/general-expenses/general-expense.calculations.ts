export function buildGeneralExpenseJournalLines(input: { amount: number; paymentMode: "CASH_BANK" | "PAYABLE"; expenseLedgerAccountId: string; bankAccountId?: string | null; partyName?: string | null }) {
  if (!(input.amount > 0)) throw new Error("Expense amount must be positive");
  if (!input.expenseLedgerAccountId) throw new Error("Expense Account ID is required");
  ...
  const debit = { accountId: input.expenseLedgerAccountId, debit: input.amount, credit: 0 };
