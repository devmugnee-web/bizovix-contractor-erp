...
import { CreateBankAccountDto, CreateCashTransactionDto, CreateChequeDto, CreatePettyExpenseDto, CreateReconciliationDto, CreateTransferDto, QueryLedgerDto, UpdateBankAccountDto, UpdateChequeStatusDto } from "./dto/cash-bank.dto";
...
export class CashBankController {
  ...
  @Patch("accounts/:id") @RequirePermissions("bank_account.update") updateAccount(@Param("id") id: string, @Body() dto: UpdateBankAccountDto, @CurrentUser() u: AuthUser) { return this.service.updateBankAccount(u.organizationId, u.id, id, dto); }
  @Delete("accounts/:id") @RequirePermissions("bank_account.update") @ResponseMessage("Bank account deleted successfully") deleteAccount(@Param("id") id: string, @CurrentUser() u: AuthUser) { return this.service.deleteBankAccount(u.organizationId, u.id, id); }
  @Get("main-cash") @RequirePermissions("cash_bank.read") mainCash(@Query() q: QueryLedgerDto, @CurrentUser() u: AuthUser) { return this.service.cashTransactions(u.organizationId, "MAIN_CASH", q); }
  ...
  @Get("petty-cash") @RequirePermissions("cash_bank.read") pettyCash(@Query() q: QueryLedgerDto, @CurrentUser() u: AuthUser) { return this.service.cashTransactions(u.organizationId, "PETTY_CASH", q); }
