...
export class ProjectClosingService {
  ...
  async overview(org: string, workId: string) {
    ...
    const [contracts, certificates, dlps, defects, releases, handovers, guarantees] = await Promise.all([
      this.prisma.projectContract.findMany({ where: { organizationId: org, cmsWorkId: workId, status: { not: "CANCELLED" } }, orderBy: { createdAt: "asc" } }),
      ...
      this.prisma.performanceGuarantee.findMany({ where: { organizationId: org, pgBgWorkflow: { cmsWork: { id: workId } } }, orderBy: { createdAt: "desc" } }),
    ]);
    return {
      contracts,
      certificates,
  ...
  async latestReadinessSnapshot(org: string, workId: string) {
    ...
    const event = await this.prisma.projectClosureEvent.findFirst({
      where: { organizationId: org, workId, action: "CLOSE", readinessSnapshot: { not: Prisma.JsonNull } },
      orderBy: { createdAt: "desc" },
  ...
  async saveCertificate(
    ...
    await this.lifecycle.assertOperationalMutationAllowed(org, workId, "changing completion records");
    await this.work(org, workId);
  ...
  async updateCertificate(org: string, userId: string, id: string, dto: UpdateCompletionCertificateDto) {
    const old = await this.prisma.completionCertificate.findFirst({ where: { id, organizationId: org } });
    if (!old) throw new NotFoundException("Completion Certificate not found");
    await this.lifecycle.assertOperationalMutationAllowed(org, old.workId, "changing completion records");
    if (old.status !== "DRAFT" && old.status !== "REJECTED") throw new BadRequestException("Only a draft or rejected certificate can be edited");
    const row = await this.prisma.completionCertificate.update({
      ...
      data: {
        ...
        ...(dto.actualCompletionDate ? { actualCompletionDate: new Date(dto.actualCompletionDate) } : {}),
        ...(dto.certifiedCompletionDate ? { certifiedCompletionDate: new Date(dto.certifiedCompletionDate) } : {}),
    ...
    await this.audit.record({ organizationId: org, userId, action: "COMPLETION_CERTIFICATE_UPDATED", entityType: "CompletionCertificate", entityId: id, oldValue: old, newValue: row });
    return row;
  }
  ...
  async certificateStatus(org: string, userId: string, id: string, dto: CertificateStatusDto) {
    ...
    const allowed: Record<string, string[]> = { DRAFT: ["SUBMITTED", "CANCELLED"], SUBMITTED: ["APPROVED", "REJECTED", "CANCELLED"], REJECTED: ["SUBMITTED", "CANCELLED"] };
    if (!allowed[old.status]?.includes(dto.status)) throw new BadRequestException(`Invalid certificate transition ${old.status} -> ${dto.status}`);
    if (dto.status === "APPROVED" && (!old.certifiedCompletionDate || !old.certificateDate || !old.issuingAuthority))
      throw new BadRequestException("Certificate date, certified completion date and issuing authority are required for approval");
    const row = await this.prisma.completionCertificate.update({
      where: { id, organizationId: org },
      data: {
        status: dto.status,
  ...
  async createDlp(org: string, userId: string, workId: string, dto: CreateDlpDto) {
    ...
    if (!durationDays || durationDays < 1) throw new BadRequestException("DLP duration is not configured on the contract");
    const startDate = new Date(dto.startDate ?? certificate.certifiedCompletionDate ?? certificate.actualCompletionDate);
  ...
  async createDefect(org: string, userId: string, workId: string, dto: CreateDefectDto) {
    ...
    await this.audit.record({ organizationId: org, userId, action: "DEFECT_CREATED", entityType: "DlpDefect", entityId: row.id, referenceNo: row.defectNo });
    return row;
  }
  ...
  async defectStatus(org: string, userId: string, id: string, dto: DefectStatusDto) {
    ...
    const allowed: Record<string, string[]> = { OPEN: ["IN_PROGRESS", "RECTIFIED"], IN_PROGRESS: ["RECTIFIED"], RECTIFIED: ["VERIFIED"], VERIFIED: ["CLOSED"], CLOSED: [] };
    if (!allowed[old.status]?.includes(dto.status)) throw new BadRequestException(`Invalid defect transition ${old.status} -> ${dto.status}`);
  ...
  async createRetentionRelease(
    ...
    await this.audit.record({ organizationId: org, userId, action: "RETENTION_RELEASE_CREATED", entityType: "RetentionRelease", entityId: row.id, referenceNo: row.releaseNo });
    return row;
  }
    ...
    const row = await this.prisma.$transaction(
      async (tx) => {
        const [bills, released] = await Promise.all([
          tx.projectBill.findMany({ where: { organizationId: org, cmsWorkId: existing.workId, status: { in: ["CERTIFIED", "PARTIALLY_RECEIVED", "RECEIVED"] } }, orderBy: { billDate: "asc" } }),
          tx.retentionRelease.aggregate({ where: { organizationId: org, workId: existing.workId, status: "RELEASED" }, _sum: { amount: true } }),
        ]);
        ...
        for (const bill of bills) {
          ...
          await tx.projectBill.update({ where: { id: bill.id, organizationId: org }, data: { retentionReleasedAmount: { increment: applied } } });
  ...
  async createHandover(org: string, userId: string, workId: string, dto: CreateHandoverDto) {
    ...
    await this.audit.record({ organizationId: org, userId, action: "HANDOVER_CREATED", entityType: "ProjectHandover", entityId: row.id, referenceNo: row.handoverNo });
    return row;
  }
  ...
  async completeHandover(org: string, userId: string, id: string) {
    ...
    await this.lifecycle.assertOperationalMutationAllowed(org, row.workId, "completing handover records");
  ...
  async readiness(org: string, workId: string) {
    ...
    const contract = await this.prisma.projectContract.findFirst({ where: { organizationId: org, cmsWorkId: workId, status: { not: "CANCELLED" } }, orderBy: { createdAt: "asc" } });
    const [certificate, finalBill, receivables, retention, dlpTotal, dlpCompleted, openDefects, handover, guarantees, workflow, payables, integrity] = await Promise.all([
      ...
      this.prisma.performanceGuarantee.findMany({ where: { organizationId: org, pgBgWorkflow: { cmsWork: { id: workId } } }, select: { id: true, status: true } }),
      contract?.pgBgWorkflowId ? this.prisma.pgBgWorkflow.findFirst({ where: { id: contract.pgBgWorkflowId, organizationId: org }, select: { pgBgRequired: true } }) : Promise.resolve(null),
      this.prisma.payable.aggregate({ where: { organizationId: org, projectId: workId, status: { not: "PAID" } }, _sum: { amount: true, paidAmount: true } }),
      this.accounting.integrity(org),
    ]);
    const outstanding = D(receivables._sum.amount ?? 0).sub(receivables._sum.receivedAmount ?? 0);
    const retentionApplicable = Boolean(contract?.retentionPct?.gt(0)) || D(retention.totalRetentionDeducted).gt(0);
    ...
    const activeGuarantees = guarantees.filter((g) => ["ACTIVE", "RELEASE_REQUESTED", "EXPIRED"].includes(g.status)).length;
    ...
    const financialBalanced = integrity.ar.status === "BALANCED" && integrity.ap.status === "BALANCED" && integrity.retention.status === "BALANCED" && integrity.banks.every((bank) => bank.status === "BALANCED") && integrity.unbalancedJournalCount === 0;
    const item = (key: string, label: string, passed: boolean, blocking: boolean, options: { applicable?: boolean; value?: string | number; message?: string; route?: string } = {}) => {
      const applicable = options.applicable ?? true;
      return { key, label, blocking: applicable ? blocking : false, passed: applicable ? passed : true, status: applicable ? (passed ? "PASSED" : blocking ? "FAILED" : "WARNING") : "NOT_APPLICABLE", value: options.value, message: options.message, route: options.route };
    };
    const items = [
      item("completion_certificate", "Completion Certificate approved", certificate > 0, true, { route: `/cms/ongoing-works/${workId}?tab=Completion%20%26%20Closeout` }),
      item("final_bill", "Final Bill certified", Boolean(finalBill), true, { route: `/cms/ongoing-works/${workId}?tab=Running%20Bills` }),
      item("receivable", "No outstanding client receivable", outstanding.lte(0), true, { value: money(outstanding) }),
      item("retention", "No outstanding retention", D(retention.outstandingRetention).lte(0), true, { applicable: retentionApplicable, value: retention.outstandingRetention }),
      item("dlp", "DLP completed", dlpCompleted > 0, true, { applicable: dlpApplicable }),
      item("defects", "No unresolved mandatory defects", openDefects === 0, true, { applicable: dlpApplicable, value: openDefects }),
      item("guarantee", "Required PG/BG released or returned", guaranteesSatisfied, true, { applicable: guaranteeApplicable, value: activeGuarantees || (guaranteeApplicable && guarantees.length === 0 ? 1 : 0), message: guaranteeApplicable && guarantees.length === 0 ? "Required guarantee has not been issued" : undefined }),
      item("handover", "Final handover completed", handover > 0, true),
      item("project_payables", "No project-specific outstanding payable", payableOutstanding.lte(0), true, { value: money(payableOutstanding) }),
      item("financial_integrity", "Financial integrity reconciled", financialBalanced, true, { message: financialBalanced ? "AR, AP, bank, retention and journals are balanced" : "Organization financial integrity is out of balance" }),
      item("security_deposit", "Security Deposit", true, false, { applicable: false, message: contract?.securityDepositPct?.gt(0) ? "NOT CONFIGURED: no authoritative Security Deposit workflow exists" : "NOT APPLICABLE" }),
    ];
    const hasBlocking = items.some((i) => i.blocking && !i.passed);
    const hasWarning = items.some((i) => i.status === "WARNING" || (i.status === "NOT_APPLICABLE" && i.message?.startsWith("NOT CONFIGURED")));
    return {
      status: hasBlocking ? "NOT_READY" : hasWarning ? "READY_WITH_WARNINGS" : "READY_TO_CLOSE",
      items,
      project: { id: work.id, status: work.status, closedAt: work.closedAt, archivedAt: work.archivedAt },
      finalBill: finalBill ? { id: finalBill.id, billNo: finalBill.billNo, status: finalBill.status, grossBillAmount: money(finalBill.grossBillAmount), netCertifiedAmount: money(finalBill.netCertifiedAmount), retentionAmount: money(finalBill.retentionAmount), totalReceived: money(finalBill.receivedAmount) } : null,
    };
  }
  ...
  async close(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (["COMPLETED", "ARCHIVED"].includes(old.status)) throw new BadRequestException("Project is already closed or archived");
      const readiness = await this.readiness(org, workId);
      const financialIntegrity = readiness.items.find((item) => item.key === "financial_integrity");
      if (financialIntegrity && !financialIntegrity.passed)
        throw new BadRequestException("Project cannot close while financial integrity is out of balance");
      if (readiness.status === "NOT_READY" && !(dto.override && canOverride && dto.reason?.trim()))
        throw new BadRequestException("Project has blocking closeout items; authorized override requires a reason");
      const closedAt = new Date();
      const changed = await tx.cmsWork.updateMany({
        where: { id: workId, organizationId: org, status: old.status },
        data: { status: "COMPLETED", completionDate: old.completionDate ?? closedAt, closedAt, closedById: userId },
      });
      if (changed.count !== 1) throw new BadRequestException("Project close state changed concurrently; refresh and retry");
      await tx.projectClosureEvent.create({
        data: {
          ...
          readinessSnapshot: { ...readiness, closedBy: userId, closedAt: closedAt.toISOString() } as Prisma.InputJsonValue,
        },
      });
      await this.audit.record({ organizationId: org, userId, action: "PROJECT_CLOSED", entityType: "CmsWork", entityId: workId, oldValue: { status: old.status }, newValue: { status: "COMPLETED", overrideReason: dto.reason, readiness } }, tx);
  ...
  async reopen(org: string, userId: string, workId: string, dto: ReopenProjectDto) {
    ...
    const row = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.cmsWork.update({
        where: { id: workId, organizationId: org },
        data: { status: "CLOSEOUT_PENDING", closedAt: null, closedById: null, archivedAt: null, archivedById: null },
      });
    });
    await this.audit.record({
      organizationId: org,
  ...
  async archive(org: string, userId: string, workId: string, dto: ArchiveProjectDto) {
    const old = await this.work(org, workId);
    if (old.status !== "COMPLETED") throw new BadRequestException("Only a closed project can be archived");
    const row = await this.prisma.$transaction(async (tx) => {
      await tx.projectClosureEvent.create({ data: { organizationId: org, workId, action: "ARCHIVE", reason: dto.reason, previousStatus: old.status, newStatus: "ARCHIVED", performedById: userId } });
      return tx.cmsWork.update({ where: { id: workId, organizationId: org }, data: { status: "ARCHIVED", archivedAt: new Date(), archivedById: userId } });
    });
    await this.audit.record({ organizationId: org, userId, action: "PROJECT_ARCHIVED", entityType: "CmsWork", entityId: workId, description: dto.reason });
    return row;
  }
