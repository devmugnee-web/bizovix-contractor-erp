...
function KpiCard({
  ...
  return (
    <div className="flex min-h-[94px] min-w-0 items-center rounded-[7px] border border-[#dfe6f1] bg-white px-3 shadow-[0_1px_2px_rgba(15,34,70,0.025)]">
      <span className={cn("flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full", tones[tone])}>
        <Icon className="h-5 w-5" />
      </span>
      <span className="ml-3 min-w-0">
        <span className="block text-[9px] font-semibold leading-tight text-[#52627d]">{label}</span>
        <span className="mt-1.5 block break-words text-[15px] font-bold leading-tight text-[#071b49]" title={value}>
          {value}
        </span>
      </span>
    </div>
  ...
  return (
    <div className="min-h-full bg-[#f8faff] px-4 pb-5 pt-3 text-[#0b1f4b] sm:px-5 xl:pl-5 xl:pr-6">
        ...
        <div className="grid grid-cols-1 items-end gap-4 md:grid-cols-2 xl:grid-cols-[1.3fr_1fr_0.9fr_1fr_auto_auto]">
          ...
          <label className="block min-w-0">
            ...
            <select
              ...
              onChange={(event) =>
                setFilterDraft((current) => ({ ...current, billType: event.target.value as BillType | "" }))
              }
              className={CONTROL_CLASS}
            >
          ...
          <label className="block min-w-0">
            ...
            <select
              ...
              onChange={(event) =>
                setFilterDraft((current) => ({ ...current, status: event.target.value as BillStatus | "" }))
              }
              className={CONTROL_CLASS}
            >
      ...
      <section className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6 xl:gap-4">
        <KpiCard
          label="Total Bills"
          value={billStats.isLoading ? "—" : statsUnavailable ? "Unavailable" : String(billStats.data?.totalBills ?? 0)}
          tone="blue"
          icon={FileText}
        />
        <KpiCard label="Gross Certified" value={moneyStat(billStats.data?.grossCertified)} tone="orange" icon={FileCheck2} />
        <KpiCard label="Net Certified" value={moneyStat(billStats.data?.netCertified)} tone="purple" icon={CircleDollarSign} />
        <KpiCard label="Received" value={moneyStat(billStats.data?.received)} tone="green" icon={WalletCards} />
        <KpiCard label="Outstanding" value={moneyStat(billStats.data?.outstanding)} tone="blue" icon={CircleDollarSign} />
        <KpiCard label="Retention Held" value={moneyStat(billStats.data?.retentionHeld)} tone="teal" icon={WalletCards} />
      </section>
      ...
      <section className="mt-4 overflow-hidden rounded-[7px] border border-[#dfe6f1] bg-white shadow-[0_1px_2px_rgba(15,34,70,0.02)]">
          ...
          <table className="w-full table-fixed text-left text-[10px] text-[#10244c]">
            ...
            <thead>
              <tr className="h-[42px] border-b border-[#e2e8f1] bg-[#fbfcfe] text-[10px] font-semibold text-[#172c53]">
                {[
                  ...
                  <th key={header} className={cn("px-2 py-2 leading-tight", header === "Action" && "text-right")}>
                    {header}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {projectBills.isLoading && (
                <tr>
                  <td colSpan={11} className="h-[110px] text-center text-[11px] text-[#6d7e99]">
              ...
              {projectBills.isError && (
                <tr>
                  <td colSpan={11} className="h-[110px] text-center text-[11px] text-[#d83b4b]">
                    <div className="flex flex-col items-center gap-2">
                      <span>Bill submissions could not be loaded.</span>
                      <button type="button" onClick={() => projectBills.refetch()} className="font-semibold text-[#075ed7] hover:underline">
                        Try again
                      </button>
                    </div>
                  </td>
              ...
              {!projectBills.isLoading &&
                ...
                rows.map((row, index) => {
                  ...
                  return (
                    <tr key={row.id} className="min-h-[48px] border-b border-[#e6ebf2] last:border-0 hover:bg-[#fbfdff]">
                      <td className="px-2 py-2.5">{(page - 1) * PAGE_SIZE + index + 1}</td>
                      <td className="break-words px-2 py-2.5 font-semibold">{row.cmsWork.tender?.egpTenderId || "—"}</td>
                      ...
                      <td className="break-words px-2 py-2.5 font-semibold">{row.billNo}</td>
                      <td className="px-2 py-2.5">{formatDate(row.billDate)}</td>
                      <td className="break-words px-2 py-2.5 font-medium">{formatBDT(row.grossBillAmount)}</td>
                      <td className="px-2 py-2.5" title={row.cmsWork.organizationMaster.shortName}>
                        <span className="line-clamp-2">{row.cmsWork.organizationMaster.shortName}</span>
                      </td>
        ...
        <div className="divide-y divide-[#e6ebf2] lg:hidden">
          {projectBills.isLoading && <div className="px-4 py-10 text-center text-[12px] text-[#6d7e99]">Loading bill submissions...</div>}
          {projectBills.isError && (
          ...
          {!projectBills.isLoading && !projectBills.isError && rows.length === 0 && (
            <div className="px-4 py-10 text-center text-[12px] text-[#6d7e99]">No bill submissions found.</div>
          )}
          {!projectBills.isLoading &&
            !projectBills.isError &&
            rows.map((row) => (
              <article key={row.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <Link href={`/cms/bills/${row.id}`} className="text-[13px] font-bold text-[#075ed7] hover:underline">
                      {row.billNo}
                    </Link>
                    <p className="mt-1 line-clamp-2 text-[12px] font-semibold text-[#10244c]">{row.cmsWork.workName}</p>
                  </div>
                  <span className={cn("shrink-0 rounded-[5px] px-2 py-[5px] text-[9px] font-semibold", STATUS_CLASS[row.status])}>
                    {BILL_STATUS_META[row.status].label}
                  </span>
                </div>
                <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
                  <div>
                    ...
                    <dd className="mt-0.5 font-medium">{row.cmsWork.tender?.egpTenderId || "—"}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="mt-0.5 font-semibold">{formatBDT(row.grossBillAmount)}</dd>
                  </div>
                </dl>
        ...
        <div className="flex min-h-[58px] flex-col items-center justify-between gap-3 border-t border-[#e2e8f1] px-3 py-3 text-[10px] text-[#40577f] sm:flex-row">
          <span>
