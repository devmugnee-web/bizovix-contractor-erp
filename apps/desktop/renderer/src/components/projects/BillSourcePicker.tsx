...
export function BillProjectReadiness({ workId }: { workId: string }) {
  ...
  const projectClosed = ["COMPLETED", "ARCHIVED", "CANCELLED"].includes(data.work.status);
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-biz-border bg-slate-50 p-3">
    ...
    <div className="flex flex-wrap gap-2 text-xs font-semibold">
      ...
      {!active.length && !projectClosed && <Link className="rounded-lg border border-biz-border bg-white px-3 py-2" href={data.contracts[0] ? `/cms/contracts/${data.contracts[0].id}/edit` : `/cms/contracts/create?cmsWorkId=${workId}`}>Set Up Contract</Link>}
...
export function BillSourcePicker({ onSelect }: { onSelect: (workId: string, selected: boolean) => void }) {
  ...
  return <section className={`mb-3 shrink-0 rounded-xl border border-biz-border bg-white p-3 shadow-sm ${selected ? "xl:max-h-[42vh] xl:overflow-y-auto" : ""}`}>
    {selected ? <>
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <label className="flex w-full min-w-0 items-center gap-2 sm:w-auto"><Search className="h-4 w-4 shrink-0 text-biz-muted" /><input aria-label="Search tender or project" className={`${control} w-full sm:w-72`} placeholder="Search Tender ID / Work Name" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} /></label>
      </div>
      {query.isError ? <p className="py-4 text-center text-xs text-biz-danger">Could not load tenders / projects. <button className="underline" onClick={() => void query.refetch()}>Retry</button></p> : query.isLoading ? <p className="py-4 text-center text-xs text-biz-muted">Loading…</p> : !query.data?.items.length ? <p className="py-4 text-center text-xs text-biz-muted">{kind === "tenders" ? "No saved, costed tenders found." : "No projects found."}</p> : <div className="divide-y divide-biz-border">{query.data.items.map((source) => <button key={source.id} onClick={() => choose(source)} className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left text-xs hover:bg-blue-50 focus-visible:outline-biz-blue">
        <span className="w-20 shrink-0 font-semibold text-biz-blue">{source.tenderNumber ?? "Project"}</span>
        <span className="min-w-0 flex-1"><span className="line-clamp-2 font-medium leading-snug" title={source.name}>{source.name}</span><span className="mt-0.5 block truncate text-[11px] text-biz-muted">{source.organization}</span></span>
