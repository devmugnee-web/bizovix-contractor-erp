  ...
  return (
    <ul
      className={cn(
        ...
        nested ? "ml-3 mt-1 border-white/10 pl-4" : "ml-[22px] mt-1.5 border-white/15 pl-5",
      )}
    >
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            <span
              className={cn(
                "absolute -left-[17px] top-[18px] h-1.5 w-1.5 -translate-y-1/2 rounded-full transition-all duration-150",
                active || childActive
            ...
            {hasChildren ? (
              <>
                {item.href ? (
                  ...
                  <span className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-[13px] font-semibold text-white">
                    <Icon className="h-[15px] w-[15px] shrink-0" />
                    <span className="min-w-0 flex-1 truncate">{item.label}</span>
                    <ChevronDown className="h-3.5 w-3.5 shrink-0 rotate-180" />
                  </span>
